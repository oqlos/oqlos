...
class CoreS3HttpClient:
    ...
    def _request(self, path: str, body: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if body is not None and self.capability_client is not None:
            ...
            headers["Authorization"] = (
                f"Bearer {self.capability_client.token(capability, resource)}"
            )
        ...
        for attempt in range(2):
            try:
                ...
                # ESP-IDF closes request sockets without always returning an
                # explicit Connection header. Reusing that half-closed socket
                # makes the next lease/output request wait for the full timeout.
                self.close()
                if status >= 400:
                    raise RuntimeError(
    ...
    def _authorization_scope(command: str, params: dict[str, Any]) -> tuple[str, str]:
        ...
        if command == "config_apply":
            return (
                "hardware.node.network.identity.configure",
                "hardware-node:stacknet/network",
            )
        if command == "set_coil" and isinstance(params.get("coil"), int):
            return (
                "hardware.node.output.set",
                f"hardware-node:stacknet/output/{params['coil'] + 1}",
            )
    ...
    def execute(self, command: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if command in {"set_coil", "config_apply"} and self.lease_id:
            effective_params.setdefault("lease_id", self.lease_id)
        return self._request(
            "/api/v1/oql", {"command": command, "params": effective_params}
        )
    ...
    def acquire_lease(self, lease_id: str, ttl_ms: int) -> dict[str, Any]:
        payload = self._request(
            "/api/v1/oql",
            {
                "command": "lease_acquire",
                "params": {"lease_id": lease_id, "ttl_ms": ttl_ms},
            },
        )
    ...
    def renew_lease(self) -> dict[str, Any]:
        if not self.lease_id:
            raise RuntimeError("Cannot renew an inactive StackNet lease")
        return self._request(
            "/api/v1/oql",
            {
                "command": "lease_renew",
                "params": {"lease_id": self.lease_id, "ttl_ms": self.lease_ttl_ms},
            },
        )
