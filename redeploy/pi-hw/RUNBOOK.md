# pi-hw provisioning runbook (192.168.188.110)

One-time bare-metal setup for the dedicated OqlOS hardware Raspberry Pi 3. Run these
**before** the first `redeploy run redeploy/pi-hw/migration.md`. After provisioning, all
...
- Hostname `pi-hw`; user `pi`.
- Wired **ethernet** (Modbus-over-network latency: avoid Wi-Fi).
- Reserve **192.168.188.110** for the Pi's MAC in the router DHCP table (static lease).

## 2. SSH access
```bash
ssh-copy-id pi@192.168.188.110          # from the laptop/pi109 that runs redeploy
ssh pi@192.168.188.110 true             # verify key-only login works
```

## 3. Base packages
```bash
ssh pi@192.168.188.110 'sudo apt-get update && \
  sudo apt-get install -y python3-venv python3-pip mosquitto mosquitto-clients'
```
If the RTC HAT is fitted, enable I2C:
```bash
ssh pi@192.168.188.110 'sudo raspi-config nonint do_i2c 0'
```

## 4. systemd --user + device groups
```bash
ssh pi@192.168.188.110 'sudo loginctl enable-linger pi && \
...
```bash
ssh pi@192.168.188.110 'mkdir -p ~/maskservice/config && \
...
USB hub on pi-hw with: Pololu Tic T249 (`1ffb:00c9`), Waveshare Modbus IO 8CH (`1a86`),
...
## 8. Verify
```bash
ssh pi@192.168.188.110 'systemctl --user is-active mosquitto oqlos-hardware-api hw-tic249 dri0050-motor-api pirtc-api'
ssh pi@192.168.188.110 'mosquitto_sub -u oqlos -P "<token>" -t "\$SYS/broker/uptime" -C 1'
...
`192.168.188.110` (`OQLOS_API_URL` + `OQLOS_OQL_MQTT_HOST`). See the **Hardware Separation**
...
If pi-hw is unavailable, redeploy pi109 **without** the flag
