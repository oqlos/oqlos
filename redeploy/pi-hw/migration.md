# OqlOS hardware node — pi-hw (192.168.188.110) deploy

Deploys the OqlOS **hardware runtime** plus an **MQTT broker** onto a dedicated
Raspberry Pi 3 (`pi@192.168.188.110`). This Pi owns all physical devices (Modbus IO/ADC,
Pololu Tic T249, DFRobot DRI0050, RTC HAT) and runs the OQL-over-MQTT **agent**: it
subscribes to `oqlos/c2004/pi-hw/oql/request`, executes the OQL/manage verb against the
...
PIHW_ALLOW_MISSING_HARDWARE=1 redeploy run redeploy/pi-hw/migration.md
```
...
echo "PASS: katalogi pi-hw utworzone"
```
...
cp /home/pi/maskservice/pi-hw-config/mosquitto.conf /home/pi/maskservice/config/mosquitto.conf
...
cp /home/pi/maskservice/pi-hw-config/oqlos-hw.yaml /home/pi/maskservice/config/oqlos-real.yaml
cp /home/pi/maskservice/pi-hw-config/.env.hw /home/pi/maskservice/config/oql-mqtt.env 2>/dev/null || true
...
[Unit]
Description=OqlOS hardware node + OQL-over-MQTT agent (pi-hw)
...
CORR="assert-$(date +%s)"
REQ="{\"correlation_id\":\"$CORR\",\"oql\":\"\",\"kind\":\"ping\",\"reply_to\":\"oqlos/c2004/pi-hw/oql/response/$CORR\"}"
mosquitto_sub -h 127.0.0.1 -p 1883 -u oqlos -P "$PW" \
  -t "oqlos/c2004/pi-hw/oql/response/$CORR" -C 1 -W 8 > /tmp/oql_pong.json 2>/dev/null &
SUB=$!
sleep 1
mosquitto_pub -h 127.0.0.1 -p 1883 -u oqlos -P "$PW" -t "oqlos/c2004/pi-hw/oql/request" -m "$REQ"
...
extra_steps:
  ...
  - id: mkdir_hw_remote
    ...
    description: "Utwórz katalogi na pi-hw"
  ...
  - id: sync_oqlos_core
    action: rsync
    description: "Sync oqlos core na pi-hw"
  ...
  - id: sync_pihw_config
    action: rsync
    description: "Sync konfiguracji węzła pi-hw (mosquitto.conf, oqlos-hw.yaml, .env.hw)"
    src: /home/tom/github/oqlos/oqlos/redeploy/pi-hw/
    dst: ~/maskservice/pi-hw-config/
    excludes: [.git/]
