...
def test_oqlos_error_fastapi_handler_returns_standard_body():
    ...
    body = resp.json()
    assert body["code"] == "serial_port_busy"
    assert body["domain"] == "hardware"
    assert body["detail"] == {"port": "/dev/ttyUSB0"}
    assert body["repair"]["id"] == "release_serial_port"
...
def test_oqlos_error_handler_can_be_installed_on_router_only_test_app():
    ...
    body = resp.json()
    assert body["code"] == "api_oql_transport_disabled"
    assert body["domain"] == "api"
    assert body["repair"]["id"] == "enable_oql_mqtt_transport"
