...
def _merge_paired_if(
    indent: str, sensor: str, lo: str, hi: str, lo2: str, hi2: str
) -> list[str]:
    """Merge two complementary single-sided IFs on the same sensor into a range."""
    ...
    return [
        f"{indent}IF '{sensor}' '{range_str}'",
        f"{indent}  CORRECT '{sensor} w zakresie {range_str}'",
        f"{indent}  ERROR '{sensor} poza zakresem {range_str}'",
    ]


def _rewrite_single_sided_if(indent: str, sensor: str, lo: str, hi: str) -> list[str]:
    """Rewrite a single-sided IF with 999999 sentinel into a range form."""
    ...
    label = f"{bound} {unit}".strip() if unit else bound
    return [
        f"{indent}IF '{sensor}' '{label}'",
        f"{indent}  CORRECT '{sensor} w zakresie {label}'",
        f"{indent}  ERROR '{sensor} poza zakresem {label}'",
    ]
    ...
    while i < len(lines):
        ...
        if p:
            ...
            if p2 and p2[1] == sensor:
                result.extend(_merge_paired_if(indent, sensor, lo, hi, p2[2], p2[3]))
                i = j + 1
                continue
            result.extend(_rewrite_single_sided_if(indent, sensor, lo, hi))
            i += 1
            continue
        result.append(lines[i])
        i += 1
...
# ---------------------------------------------------------------------------
# Per-line migration handlers for migrate_v2_to_v4
# Each returns list[str] of output lines, or None if pattern didn't match.
# ---------------------------------------------------------------------------

def _mig_goal(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^GOAL\s*:\s*(.+)$", normalized, re.IGNORECASE)
    if not m:
        return None
    name = m.group(1).strip().strip("\"'")
    return ["GOAL", f"  SET NAME {_quote(name)}"]


def _mig_task(stripped: str, normalized: str) -> list[str] | None:
    if not re.match(r"^TASK\s*:\s*", normalized, re.IGNORECASE):
        return None
    ...
    if verb in {"włącz", "wlacz", "on", "enable"} and rest:
        return [f"  SET {_to_v4_token(rest[0])} 1"]
    if verb in {"wyłącz", "wylacz", "off", "disable"} and rest:
        return [f"  SET {_to_v4_token(rest[0])} 0"]
    if verb in {"zapisz", "save"} and rest:
        label = "-".join(t.replace(" ", "_") for t in rest)
        return [f"  SAVE {label}"]
    return [f"  LOG {_quote('TASK ' + raw_task)}"]


def _mig_wait(stripped: str, normalized: str) -> list[str] | None:
    if not re.match(r"^WAIT\s+", normalized, re.IGNORECASE):
        return None
    wait_raw = re.sub(r"^WAIT\s+", "", normalized, flags=re.IGNORECASE).strip()
    return [f"  WAIT {_join_value_unit(wait_raw)}"]


def _mig_sample(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(
        r"^SAMPLE\s+([^\s]+)\s+(START|STOP)(?:\s+(.+))?$", normalized, re.IGNORECASE
    )
    if not m:
        return None
    sensor = _to_v4_token(m.group(1))
    direction = m.group(2).upper()
    interval_raw = m.group(3)
    if interval_raw:
        return [f"  SAMPLE {sensor} {direction} {_join_value_unit(interval_raw)}"]
...
def _mig_delta(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^DELTA\s+(.+?)\s*=\s*(.+)$", normalized, re.IGNORECASE)
    if not m:
        return None
    sensor_v4 = _to_v4_token(m.group(1).strip())
    value = _normalize_bracket_tokens(m.group(2).strip())
...
def _mig_if_comparison(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^IF\s+(\S.*?)\s+(<|>|<=|>=|=)\s+(.+)$", stripped, re.IGNORECASE)
    if not m:
        return None
    sensor = _to_v4_token(m.group(1).strip())
    op = m.group(2)
    value = _normalize_bracket_tokens(m.group(3).strip())
    value_joined = value if value.startswith(("'", '"')) else _join_value_unit(value)
    if op in {"<", "<="}:
        return [f"  IF {sensor} -999999 .. {value_joined}"]
    if op in {">", ">="}:
        return [f"  IF {sensor} {value_joined} .. 999999"]
    return [f"  IF {sensor} {value_joined} .. {value_joined}"]


def _mig_else_error(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r'^ELSE\s+ERROR\s+"(.+)"$', normalized, re.IGNORECASE)
    if not m:
        return None
    return [f"  ERROR {_quote(m.group(1))}"]


def _mig_goto(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^GOTO\s+(.+)$", normalized, re.IGNORECASE)
    if not m:
        return None
    return [f"  LOG 'GOTO {m.group(1).strip()} - NOT SUPPORTED IN V4'"]


def _mig_save(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^SAVE\s+(.+)$", normalized, re.IGNORECASE)
    if not m:
        return None
    return [f"  SAVE {_normalize_bracket_tokens(m.group(1).strip())}"]


def _mig_else_info(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r'^ELSE\s+INFO\s+"(.+)"$', normalized, re.IGNORECASE)
    if not m:
        return None
    return [f"  LOG 'ELSE INFO: {_quote(m.group(1))} - NOT SUPPORTED IN V4'"]


def _mig_set_name(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^SET\s+NAME\s+(.+)$", normalized, re.IGNORECASE)
    if not m:
        return None
    return [f"  SET NAME {m.group(1).strip()}"]


def _mig_set_eq(stripped: str, normalized: str) -> list[str] | None:
    if "=" not in stripped or not re.match(r"^SET\s+", stripped, re.IGNORECASE):
        return None
    parts = stripped.split("=", 1)
    if len(parts) != 2:
        return None
    identifier = _normalize_bracket_tokens(parts[0].replace("SET", "", 1).strip())
    value = _normalize_bracket_tokens(parts[1].strip())
    value_joined = value if value.startswith(("'", '"')) else _join_value_unit(value)
    return [f"  {_format_set(identifier, _strip_outer_quotes(value_joined))}"]


def _mig_set_noeq(stripped: str, normalized: str) -> list[str] | None:
    if not re.match(r"^SET\s+", stripped, re.IGNORECASE) or "=" in stripped:
        return None
    rest = re.sub(r"^SET\s+", "", stripped, flags=re.IGNORECASE)
    bm = re.match(r"^\[([^\]]+)\]\s+(.+)$", rest)
    if bm:
        identifier, value = bm.group(1).strip(), bm.group(2).strip()
    else:
        parts = rest.split(None, 1)
        identifier = parts[0].strip() if parts else rest.strip()
        value = parts[1].strip() if len(parts) == 2 else ""
    value = _normalize_bracket_tokens(value)
    value_joined = value if value.startswith(("'", '"')) else _join_value_unit(value)
    if value_joined:
        return [f"  {_format_set(identifier, _strip_outer_quotes(value_joined))}"]
    return [f"  SET {_quote(identifier)} ''"]


def _mig_pump(stripped: str, normalized: str) -> list[str] | None:
    m = re.match(r"^PUMP\s+\[?([^\]]+)\]?$", normalized, re.IGNORECASE)
    if not m:
        return None
    value = _normalize_bracket_tokens(m.group(1).strip())
    if value.lower() in {"off", "0"}:
        return [f"  {_format_set('PUMP', '0')}"]
    return [f"  {_format_set('PUMP', _join_value_unit(value))}"]


_V2_LINE_MIGRATORS = [
    _mig_goal, _mig_task, _mig_wait, _mig_sample,
    _mig_minmax_eq, _mig_minmax_simple, _mig_delta,
    _mig_calc, _mig_val, _mig_if_comparison,
    _mig_else_error, _mig_goto, _mig_save, _mig_else_info,
    _mig_set_name, _mig_set_eq, _mig_set_noeq, _mig_pump,
]
...
def migrate_v2_to_v4(text: str) -> str:
    ...
    for raw in lines:
        ...
        handled = False
        for migrator in _V2_LINE_MIGRATORS:
            result = migrator(stripped, normalized)
        ...
        if not handled:
            if re.match(
                r"^(SCENARIO|VERSION|GOAL:|CONFIG|MACRO|FUNC:)\b",
                normalized,
                re.IGNORECASE,
            ):
    ...
    first_meaningful = next(
        (x for x in out if x.strip() and not x.strip().startswith("#")), None
    )
    if not first_meaningful or not re.match(
        r"^VERSION\s*:\s*4\s*$", first_meaningful, re.IGNORECASE
    ):
...
def _process_row(
    ...
    scenario_dir: Path,
    prefer_local: bool,
) -> tuple[MigrationResult, str | None, dict[str, Any] | None]:
    """Migrate one scenario row; return (result, migrated_code_or_None_if_unchanged, preview_entry_or_None)."""
    ...
    return result, migrated_code, preview


def _apply_row_update(
    row: dict[str, Any],
    migrated_code: str,
    write_url_template: str,
    method: str,
) -> dict[str, Any]:
    """Send migrated code to the remote write endpoint; return result dict."""
    sid = str(row.get("id") or "").strip()
    write_url = _build_write_url(write_url_template, sid)
    payload = _build_write_payload(row, migrated_code)
    try:
        response = _send_json(write_url, method, payload)
        return {"id": sid, "write_url": write_url, "response": response}
...
def main() -> int:
    ...
    for row in rows:
        ...
        if not sid:
            continue
        result, migrated_code, preview = _process_row(row, scenario_dir, args.prefer_local)
