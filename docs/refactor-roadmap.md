...
Bieżący stan wykonawczy po dziesiątej grupie `NEXT-04`: commit `54df6cd`, 194
trasy, 80 zwrotów `dict[str, Any]`, 182 generyczne odpowiedzi OpenAPI, 78
surowych wyjątków, 173 szerokie handlery, 154 odczyty env, 30 dużych modułów
i 0 negatywnych odpowiedzi przy HTTP 200. Lokalna bramka: 1107/1107 backend,
...
`correlation_id` i nie odbija nazwy jednostki podanej przez klienta.

Kolejne dwa surowe 403 w zapisie konfiguracji sprzętu i fizycznym impulsie
cewki Modbus zostały zastąpione lokalnymi kodami mapowanymi na
`C2004-AUTH-0002`. Obie granice zatrzymują żądanie przed zapisem/aktuacją,
zachowują `correlation_id` i nie publikują odrzuconej roli ani payloadu.

Cztery odmowy ścieżek w API edytora używają teraz
403/`C2004-AUTH-0002` i lokalnego `api_editor_path_forbidden` bez publikowania
...
Walidacja `StartExecution` w `state.py` zwraca teraz 422/`C2004-DATA-0002`
dla brakującego źródła, błędnego DSL i DSL bez kroków, a brak scenariusza
zwraca bezpieczne 404/`C2004-DATA-0001`. Nieznana komenda nie wraca już jako
...
`execution.py` nie używa już surowych `HTTPException`. Błędny DSL i payload
kroku dają 422/`C2004-DATA-0002`, brak scenariusza lub wykonania daje
404/`C2004-DATA-0001`, a legacy control bez bieżącego wykonania zwraca
409/`C2004-DATA-0003`. Nieznana awaria orkiestratora trafia do sanitizowanej
granicy 500/`C2004-SYS-0000`, zamiast być błędnie opisana jako HTTP 400.

`editor.py` nie używa już surowych `HTTPException` ani szerokich lokalnych
handlerów. Brak pliku daje 404/`C2004-DATA-0001`, wskazanie katalogu zamiast
pliku daje 422/`C2004-DATA-0002`, a naruszenie sandboxa pozostaje
403/`C2004-AUTH-0002`. Nieoczekiwane awarie przechodzą przez sanitizowaną
granicę 500/`C2004-SYS-0000`, bez publikowania ścieżki lub tekstu wyjątku.

Trasy `hardware-configuration` nie używają już surowych `HTTPException`.
Brak, nieczytelny lub błędny aktywny plik daje 503/`C2004-HW-0012`, a błędna
treść żądania, nazwa celu lub niezgodność rozszerzenia daje
...
Ósma grupa sklasyfikowała wszystkie sześć szerokich handlerów legacy adaptera
firmware. Błędy transportu, zależności, payloadu i odrzucenia komendy mają
typowane klasy, a wynik komendy publikuje stabilny 503/`C2004-HW-0012` bez
...
Dziesiąta grupa sklasyfikowała plugin artificial-lung i współdzielony dekoder
odpowiedzi pluginów HTTP. Oczekiwane błędy transportu, runtime i payloadu mają
stabilny envelope, nie publikują komunikatu wyjątku ani wejściowej komendy, a
defekty programu nie są maskowane. Niepoprawny JSON statusu runtime blokuje
aktuację zamiast być uznany za stan zdrowy. Audyt wskazuje 173 szerokie
handlery, 78 surowych wyjątków i 30 dużych modułów; `lung.py` ma 395 linii, a
wspólna polityka granicy 89. Bramka: 1107/1107 backend, 149/149 frontend,
build Vite, Ruff i `uv lock --check`.

Pozostały zakres: sklasyfikować 78 surowych wyjątków i 173 szerokie handlery
...
wdrożeniowej. `NEXT-04` trwa; po dziesięciu grupach następny aktywny zakres to
pluginy sprzętowe `modbus`, `modbus_adc` i `piadc`. Legacy `gateway.py` wymaga
