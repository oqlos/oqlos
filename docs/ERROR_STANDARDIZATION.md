...
| Waveshare diagnose: zajęty adapter | 409 | `C2004-HW-0013` | `SOA/firmware` | `modbus-waveshare` / `matrix.scan` | sanitizowany `serial-device://<label>`; wyjątek i ścieżka nie są publikowane |
...
| lifecycle rejestru pluginów zawodzi | 503 | `C2004-HW-0012` | `SOA/firmware` | `plugin-registry` / connect, disconnect albo health | stabilny komunikat i wyłącznie typ wyjątku w logu; zewnętrzny entry point jest celową szeroką granicą, bez nazwy, ścieżki i komunikatu wyjątku |
| plugin artificial-lung: oczekiwana awaria connect, health albo komendy | 503 | `C2004-HW-0012` | `SOA/firmware` | `motor-tic249` / `adapter.execute` | stabilny `reason`; bez tekstu wyjątku, ścieżki i sekretu, w logu wyłącznie typ wyjątku |
| plugin artificial-lung: błędny status runtime | 503 | `C2004-HW-0012` | `SOA/firmware` | `motor-tic249` / `adapter.preflight` | aktuacja jest blokowana; surowy payload statusu nie trafia do wyniku |
| plugin artificial-lung: defekt programu | 500 na granicy publicznej | `C2004-SYS-0000` | `SOA/oqlos` | `oqlos-api` / `api.error` | `AttributeError` nie jest maskowany jako awaria urządzenia |
| gateway pluginów: oczekiwana awaria init/reconnect/readiness/komendy | obserwacja stabilna albo 503 na granicy trasy | właściwy katalogowy `C2004-*` | `SOA/firmware` | `plugin-gateway` / etap operacji | wyłącznie stabilny `reason`; brak tekstu wyjątku, surowego health, surowego powodu init i negatywnego payloadu pluginu; nieznany kod upstream jest odrzucany |
| gateway pluginów: defekt programu | 500 na granicy publicznej | `C2004-SYS-0000` | `SOA/oqlos` | `oqlos-api` / `api.error` | `AttributeError/TypeError` nie są maskowane jako niedostępny sprzęt; komunikat pozostaje poza odpowiedzią |
| legacy adapter firmware: transport, zależność, błędny payload lub odrzucenie komendy | 503 | `C2004-HW-0012` | `SOA/firmware` | `firmware-adapter` / `command.execute` | `hardware-runtime://firmware-adapter`; bez hosta, payloadu i komunikatu wyjątku; defekt programu przechodzi do bezpiecznej 500 |
